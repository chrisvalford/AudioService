...
let STK_DISABLE_BUFFER = (0xffffffff)

class STKAudioPlayer : NSObject, STKDataSourceDelegate {
...
func play:(NSString*)urlString;

/// Plays an item from the given URL (all pending queued items are removed)
func play:(NSString*)urlString withQueueItemID:(NSObject*)queueItemId;

/// Plays an item from the given URL (all pending queued items are removed)
/// The NSURL is used as the queue item ID
func playURL:(NSURL*)url;

/// Plays an item from the given URL (all pending queued items are removed)
func playURL:(NSURL*)url withQueueItemID:(NSObject*)queueItemId;

/// Plays the given item (all pending queued items are removed)
/// The STKDataSource is used as the queue item ID
func playDataSource:(STKDataSource*)dataSource;

/// Plays the given item (all pending queued items are removed)
func playDataSource:(STKDataSource*)dataSource withQueueItemID:(NSObject*)queueItemId;

/// Queues the URL string for playback and uses the NSString as the queueItemID
func queue:(NSString*)urlString;

/// Queues the URL string for playback with the given queueItemID
func queue:(NSString*)urlString withQueueItemId:(NSObject*)queueItemId;

/// Queues the URL for playback and uses the NSURL as the queueItemID
func queueURL:(NSURL*)url;

/// Queues the URL for playback with the given queueItemID
func queueURL:(NSURL*)url withQueueItemId:(NSObject*)queueItemId;

/// Queues a DataSource with the given queueItemId
func queueDataSource:(STKDataSource*)dataSource withQueueItemId:(NSObject*)queueItemId;

/// Plays the given item (all pending queued items are removed)
func setDataSource:(STKDataSource*)dataSourceIn withQueueItemId:(NSObject*)queueItemId;

/// Seeks to a specific time (in seconds)
func seekToTime:(double)value;
...
func removeFrameFilterWithName:(NSString*)name;

/// Appends a frame filter with the given name and filter block to the end of the filter chain
func appendFrameFilterWithName:(NSString*)name block:(STKFrameFilter)block;
...
func addFrameFilterWithName:(NSString*)name afterFilterWithName:(nullable NSString*)afterFilterWithName block:(STKFrameFilter)block;
...
var STKAudioPlayerInternalState internalState;
var STKAudioPlayerInternalState stateBeforePaused;

func handlePropertyChangeForFileStream:(AudioFileStreamID)audioFileStreamIn fileStreamPropertyID:(AudioFileStreamPropertyID)propertyID ioFlags:(UInt32*)ioFlags;
func handleAudioPackets:(const void*)inputData numberBytes:(UInt32)numberBytes numberPackets:(UInt32)numberPackets packetDescriptions:(AudioStreamPacketDescription*)packetDescriptions;
//@end
...
-(STKAudioPlayerInternalState) internalState
{
    ...
    func setInternalState(value: STKAudioPlayerInternalState)
{
    self.setInternalState(value, ifInState:NULL)
}

    func setInternalState(value: STKAudioPlayerInternalState) ifInState:(BOOL(^)(STKAudioPlayerInternalState))ifInState
{
...
func startSystemBackgroundTask
{
#if TARGET_OS_IPHONE
...
func queueDataSource:(STKDataSource*)dataSourceIn withQueueItemId:(NSObject*)queueItemId
{
    pthread_mutex_lock(&playerMutex);
    {
        if (![self audioGraphIsRunning])
...
func handlePropertyChangeForFileStream:(AudioFileStreamID)inAudioFileStream fileStreamPropertyID:(AudioFileStreamPropertyID)inPropertyID ioFlags:(UInt32*)ioFlags
{
    OSStatus error;
...
func unexpectedError:(STKAudioPlayerErrorCode)errorCodeIn
{
...
func audioQueueFinishedPlaying:(STKQueueEntry*)entry
{
...
func setCurrentlyReadingEntry:(STKQueueEntry*)entry andStartPlaying:(BOOL)startPlaying
{
    [self setCurrentlyReadingEntry:entry andStartPlaying:startPlaying clearQueue:YES];
}

func setCurrentlyReadingEntry:(STKQueueEntry*)entry andStartPlaying:(BOOL)startPlaying clearQueue:(BOOL)clearQueue
{
...
func processFinishPlayingIfAnyAndPlayingNext:(STKQueueEntry*)entry withNext:(STKQueueEntry*)next
{
...
func dispatchSyncOnMainThread:(void(^)(void))block
{
...
func playbackThreadQueueMainThreadSyncBlock:(void(^)(void))block
{
    block = [block copy];
...
func dataSourceDataAvailable:(STKDataSource*)dataSourceIn
{
    OSStatus error;
...
func dataSourceErrorOccured:(STKDataSource*)dataSourceIn
{
    if (currentlyReadingEntry.dataSource != dataSourceIn)
    {
        return;
...
func dataSourceEof:(STKDataSource*)dataSourceIn
{
...
func createAudioConverter:(AudioStreamBasicDescription*)asbd
{
    OSStatus status;
    Boolean writable;
    UInt32 cookieSize = 0;
...
func connectNodes:(AUNode)srcNode desNode:(AUNode)desNode srcUnit:(AudioComponentInstance)srcUnit desUnit:(AudioComponentInstance)desUnit
{
    OSStatus status;
...
func setOutputCallbackForFirstNode:(AUNode)firstNode firstUnit:(AudioComponentInstance)firstUnit
{
    OSStatus status;
...
func stopAudioUnitWithReason:(STKAudioPlayerStopReason)stopReasonIn
{
    OSStatus status;
...
func handleAudioPackets:(const void*)inputData numberBytes:(UInt32)numberBytes numberPackets:(UInt32)numberPackets packetDescriptions:(AudioStreamPacketDescription*)packetDescriptionsIn
{
    if (currentlyReadingEntry == nil)
    {
        return;
...
func setPeakPowerInDecibelsForChannel:(NSUInteger)channelNumber andPower: (Float32)power
{
    if (channelNumber >= canonicalAudioStreamBasicDescription.mChannelsPerFrame)
    {
        return;
...
func setAveragePowerInDecibelsForChannel:(NSUInteger)channelNumber andPower: (Float32)power
{
    if (channelNumber >= canonicalAudioStreamBasicDescription.mChannelsPerFrame)
    {
        return;
...
func setMeteringEnabled:(BOOL)value
{
...
func addFrameFilterWithName:(NSString*)name afterFilterWithName:(NSString*)afterFilterWithName block:(STKFrameFilter)block
{
    pthread_mutex_lock(&self->playerMutex);
...
func addFrameFilter:(STKFrameFilter)frameFilter withName:(NSString*)name afterFilterWithName:(NSString*)afterFilterWithName
{
    pthread_mutex_lock(&self->playerMutex);
